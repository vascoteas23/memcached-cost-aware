...
static unsigned int lru_type_map[5] = { HOT_LRU_R, HOT_LRU_L, WARM_LRU,
COLD_LRU, TEMP_LRU };

#define LARGEST_ID POWER_LARGEST // modified it was POWER_LARGEST not 300
typedef struct {
	uint64_t evicted;
...
//static item *heads_h[LARGEST_ID];
static item *heads[LARGEST_ID];
static item *tails[LARGEST_ID];
//static item *tails_h[LARGEST_ID];
...
item *do_item_alloc_pull_priority(const size_t ntotal, const unsigned int id,
		const unsigned short priority, const int flag, char *key,
		const size_t nkey) {
	item *it = NULL;
	int i;
	/* If no memory is available, attempt a direct LRU juggle/eviction */
		...
		if (it == NULL) {
			...
			if (priority > return_minimum_priority_slclass(id | HOT_LRU_L)) {
				...
				NULL) <= 0) {
					...
					if (do_run_search_thread == 0
							&& settings.slab_automove == 3) {
...
item *do_item_alloc(char *key, const size_t nkey, const unsigned short cost,
		const unsigned int flags, const rel_time_t exptime, const int nbytes) {
	uint8_t nsuffix;
	item *it = NULL;
	char suffix[40];
...
//		}
	...
	if (ntotal > settings.slab_chunk_size_max) {
		...
		it = do_item_alloc_pull_priority(htotal, hdr_id, priority, nbytes, key,
				nkey);
		/* setting ITEM_CHUNKED is fine here because we aren't LINKED yet. */
		if (it != NULL)
			it->it_flags |= ITEM_CHUNKED;
	} else {
		it = do_item_alloc_pull_priority(ntotal, id, priority, nbytes, key,
				nkey);
	}
...
//					ori_id = it->slabs_clsid - HOT_LRU_L;
			...
			if ((priority > return_minimum_priority_slclass(ori_id))
					&& (it->slabs_clsid < HOT_LRU_L
...
char *item_cachedump(const unsigned int slabs_clsid, const unsigned int limit,
	...
	while (it != NULL && (limit == 0 || shown < limit)) {
		...
		len = snprintf(temp, sizeof(temp), "ITEM %s [%d b; %llu s]\r\n",
				...
				it->exptime == 0 ?
						0 : (unsigned long long )it->exptime + process_started);
...
//								}
	...
	if (settings.lru_maintainer_thread) {
		...
		APPEND_STAT("inHot", "%llu", (unsigned long long )totals.inHot);
...
//				}
		...
		if (settings.lru_maintainer_thread) {
			...
			if (settings.temp_lru) {
				APPEND_NUM_FMT_STAT(fmt, n, "number_temp", "%u",
						lru_size_map[3]);
			}
...
item *do_item_get(const char *key, const size_t nkey, const uint32_t hv,
	...
	LOGGER_LOG(c->thread->l, LOG_FETCHERS, LOGGER_ITEM_GET, NULL, was_found,
			key, nkey, (it) ? ITEM_clsid(it) : 0);
...
//	fprintf(stderr, "entrei");
	item ** reserved_head = &heads[orig_id];
	item ** reserved_tail = &tails[orig_id];
	if((*reserved_head) != NULL){
	fprintf(stderr, "head %s", ITEM_key(*reserved_head));
	}
	short tmp_min = 0;
	if (*reserved_head == *reserved_tail && *reserved_head != NULL) {
		fprintf(stderr, "tail, priority %d", (*reserved_head)->priority);
		if (((*reserved_head)->priority < tmp_min
				&& (*reserved_head)->priority
						> return_minimum_priority_slclass(orig_id)
				&& (*reserved_head)->priority == 0) || tmp_min == 0) {
			tmp_min = (*reserved_head)->priority;
		}
		if ((*reserved_head)->priority
				< return_minimum_priority_slclass(orig_id) && return_minimum_priority_slclass(orig_id) != 0) {
			do_item_unlink_q(*reserved_head);
			pthread_mutex_lock(&lru_locks[orig_id]);
			(*reserved_head)->slabs_clsid = ITEM_clsid(*reserved_head);
			(*reserved_head)->slabs_clsid |= COLD_LRU;
			item_link_q(*reserved_head);
			pthread_mutex_unlock(&lru_locks[orig_id]);
		}
		return 0;
	}
	while (*reserved_head != *reserved_tail && *reserved_head != NULL) {
		item ** tmp = &(*reserved_head)->next;
//		fprintf(stderr, "next H %s,priority %d minprio %d, size %d, tail%s \n", ITEM_key(*reserved_head),
//				(*reserved_head)->priority,return_minimum_priority_slclass(orig_id), sizes[orig_id],ITEM_key(*reserved_tail));
		if (((*reserved_head)->priority < tmp_min
				&& (*reserved_head)->priority
						> return_minimum_priority_slclass(orig_id)) || tmp_min == 0) {
			tmp_min = (*reserved_head)->priority;
		}
		if ((*reserved_head)->priority
				< return_minimum_priority_slclass(orig_id) && return_minimum_priority_slclass(orig_id) != 0) {
			pthread_mutex_lock(&lru_locks[orig_id]);
			fprintf(stderr,"removing prio %d min %d",(*reserved_head)->priority,return_minimum_priority_slclass(orig_id));
			do_item_unlink_q(*reserved_head);
			(*reserved_head)->slabs_clsid = ITEM_clsid(*reserved_head);
			(*reserved_head)->slabs_clsid |= COLD_LRU;
			item_link_q(*reserved_head);
...
//		fprintf(stderr, "cheguei");
		//fprintf(stderr,"finish, head %s, tail %s",ITEM_key(*reserved_head),ITEM_key(*reserved_head));
	}
	fprintf(stderr,"tmp %d",tmp_min);
	minimum_priority_slclass(orig_id, (float)(tmp_min/10));
	return 0;
}
	...
	for (; tries > 0 && search != NULL; tries--, search = next_it) {
		...
		if (refcount_incr(search) != 2) {
			...
			if (settings.tail_repair_time
					&& search->time + settings.tail_repair_time
							< current_time) {
...
//							cold_tries--, cold_search = cold_next_it) {
					...
					uint32_t hv = hash(ITEM_key(cold_search),
							cold_search->nkey);
...
static bool lru_bump_async(lru_bump_buf *b, item *it, uint32_t hv) {
	...
	lru_bump_entry *be = (lru_bump_entry *) bipbuf_request(b->buf,
			sizeof(lru_bump_entry));
	if (be != NULL) {
		be->it = it;
		be->hv = hv;
...
static void lru_maintainer_crawler_check(struct crawler_expired_data *cdata,
	...
	for (i = POWER_SMALLEST; i < POWER_LARGEST; i++) {
		...
		if (s->run_complete) {
			...
			LOGGER_LOG(l, LOG_SYSEVENTS, LOGGER_CRAWLER_STATUS, NULL,
					CLEAR_LRU(i), lru_name, (unsigned long long )low_watermark,
	...
	if (do_run) {
		...
		lru_crawler_start(todo, tocrawl_limit, CRAWLER_AUTOEXPIRE, cdata,
		NULL, 0);
	}
}

slab_automove_reg_t slab_automove_default = { .init = slab_automove_init,
		.free = slab_automove_free, .run = slab_automove_run };
#ifdef EXTSTORE
...
//		short miss_counter = 0;
	...
	struct crawler_expired_data *cdata = calloc(1,
			sizeof(struct crawler_expired_data));
	if (cdata == NULL) {
		fprintf(stderr,
	...
	if (l == NULL) {
		fprintf(stderr,
				"Failed to allocate logger for LRU maintainer thread\n");
		abort();
	}
	...
	while (do_run_lru_maintainer_thread) {
		...
		for (i = POWER_SMALLEST; i < MAX_NUMBER_OF_SLAB_CLASSES; i++) {
			next_juggles[i] =
					next_juggles[i] > last_sleep ?
							next_juggles[i] - last_sleep : 0;
		...
		if (settings.lru_segmented && lru_maintainer_bumps()
				&& to_sleep > 1000) {
			to_sleep = 1000;
		}
...
static void *control_thread(void *arg) {
	...
	while(true){
		for (int i = POWER_SMALLEST; i < MAX_NUMBER_OF_SLAB_CLASSES; i++) {
			if (slcls_num_slabs(i) > 0) {
				cost_control(i | HOT_LRU_L);
			}
		}
		sleep(20);
	}
...
static void *slope_thread(void *arg) {
	...
	while (do_run_slope_thread) {
		...
		if (counter == 0)
			continue;
		fprintf(stderr, "counter %d, content %d", globalhitratecounter,
				hitratearray[globalhitratecounter]);
//			if(do_reset == 5){
...
//					LOGGER_LOG(NULL, LOG_MISSES, LOGGER_GET_MISSES, it);
				...
				slabs_reassign(return_min_cost_per_byte(),
						return_max_cost_per_byte()); // trocar o src com o dst
				reset_hitratearray();
				pointer1 = hitratearray;
				pointer2 = pointer1 + 4;
				tmp = pointer1 + 1;
...
int start_slope_thread(void *arg) {
	...
	if (((ret = pthread_create(&slope_tid, NULL, slope_thread, arg)) != 0)) {
...
int start_control_thread(void *arg) {
	...
	do_run_control_thread = 1;
	if (((ret = pthread_create(&control_tid, NULL, control_thread, arg)) != 0)) {
		fprintf(stderr, "Can't create slope thread: %s\n", strerror(ret));
...
int start_search_thread(void *arg) {
	...
	if ((ret = pthread_create(&search_tid, NULL, search_thread, arg)) != 0) {
