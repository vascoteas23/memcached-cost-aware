...
//	}
	...
	bool item_size_ok(const size_t nkey, const int flags, const int nbytes) {
		...
		size_t ntotal = item_make_header(nkey + 1, flags, nbytes, prefix,
				&nsuffix);
		if (settings.use_cas) {
			ntotal += sizeof(uint64_t);
		}
...
//				}
			...
			if (settings.lru_maintainer_thread) {
				...
				APPEND_NUM_FMT_STAT(fmt, n, "number_warm", "%u",
						lru_size_map[1]);
				APPEND_NUM_FMT_STAT(fmt, n, "number_cold", "%u",
						lru_size_map[2]);
				if (settings.temp_lru) {
			...
			APPEND_NUM_FMT_STAT(fmt, n, "evicted_time", "%u",
					totals.evicted_time);
...
//		fprintf(stderr,"count %d", removed);
	...
	static int lru_maintainer_juggle(const int slabs_clsid) {
		...
		slabs_available_chunks(slabs_clsid, NULL, &total_bytes,
				&chunks_perslab);
		if (settings.temp_lru) {
