...
static void process_update_command(conn *c, token_t *tokens, const size_t ntokens, int comm, bool handle_cas) {
    ...
    if (tokens[KEY_TOKEN].length > KEY_MAX_LENGTH) {
        out_string(c, "CLIENT_ERROR bad command line format this 1");
        return;
    }
    ...
    if (handle_cas) {
        if (!safe_strtoull(tokens[6].value, &req_cas_id)) {
            out_string(c, "CLIENT_ERROR bad command line format this 3");
            return;
        }
    }

    if (vlen < 0 || vlen > (INT_MAX - 2)) {
        out_string(c, "CLIENT_ERROR bad command line format this 4");
        return;
    }
    vlen += 2;
